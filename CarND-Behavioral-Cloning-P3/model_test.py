...
def preprocess_image(image_path,steering, isTrainImage = True):
    ...
    if isTrainImage:
        ...
        #flip_image = grayscale(flip_image)
...
def getModel():
    ...
    model.add(Convolution2D(64,(3,3),activation="relu",kernel_regularizer=l2_reg(0.0001)))
    model.add(Flatten())
    model.add(Dense(100,kernel_regularizer=l2_reg(0.0001)))
...
train_generator = generator(X_train, y_train, True, 64)
validation_generator = generator(X_valid, y_valid, False, 64)
...
history_object = model.fit_generator(train_generator, steps_per_epoch= 3000, validation_data=validation_generator,
                    validation_steps=len(X_valid), epochs=7)
